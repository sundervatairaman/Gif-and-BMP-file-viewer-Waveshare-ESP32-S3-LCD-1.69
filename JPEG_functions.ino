...
void drawFSJpeg(const char *filename, int xpos, int ypos) {
  ...
  fs::File jpgFile = FFat.open( filename, "r");    // File handle reference for SPIFFS
  //  File jpgFile = SD.open( filename, FILE_READ);  // or, file handle reference for SD library
 
  if ( !jpgFile ) {
    tft.print("ERROR: File \""); tft.print(filename); tft.println ("\" not found!");
    return;
  }
...
void jpegInfo() {
  ...
  Serial.println(F("==============="));
  tft.print(F(  "Width      :")); Serial.println(JpegDec.width);
...
void createArray(const char *filename) {
  ...
  if ( !( jpgFile = FFat.open( filename, "r"))) {
    tft.println(F("JPEG file not found"));
    return;
  }

  uint8_t data;
  byte line_len = 0;
  tft.println("// Generated by a JPEGDecoder library example sketch:");
